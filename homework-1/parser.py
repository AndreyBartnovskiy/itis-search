...
class Parser(scrapy.Spider):
    ...
    def parse(self, response):
        ...
        if response.url in restricted_urls or url_domain in restricted_domains:
            self.log(f'Skipping URL {response.url} due to block list during parsing')
        else:
            ...
            if language.startswith('en') or language.startswith('ru'):
                ...
                self.log(
                    f'Saved new file with name="{filename}". Language="{language}". Total received {self.total_pages()} pages')

                self.increment_counters(language)

                if self.counters_reached():
                    self.log(f'Received >= {PAGES_COUNT} pages. Crawling stopped!!!')
